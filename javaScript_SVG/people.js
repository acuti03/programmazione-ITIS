...
class Person extends Circle {
    ...
    #speed = speed / 3 + Math.random() * speed; 
    #acc = 0;
    ...
    hitsPerson = function(p) {
        const distance = Math.sqrt(Math.pow(this.x - p.x, 2) + Math.pow(this.y - p.y, 2));
...
class People extends Array {
    ...
    #addPerson = function(n) {
        ...
        if (0 != it)
            id += it;
        // create a new Person
        const person = new Person(this.#svg, id);
    ...
    /** see if there are contacts among the population */
    anyHit = (person) => {
        for (const p of this)
...
class QuintaI extends People {
    constructor(svg) {
        ...
        super(svg, alunni.length, alunni);
    }
}
